...
describe('AuditLogService – findAll', () => {
  ...
  it('filters by action when provided', async () => {
    ...
    expect(mockPrisma.auditLog.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ action: 'admin.user_status_updated' }) }),
    );
  });
  ...
  it('filters by userId when provided', async () => {
    ...
    await service.findAll({ userId: 'admin-id', page: 1, limit: 20 });

    expect(mockPrisma.auditLog.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ userId: 'admin-id' }) }),
    );
  });
